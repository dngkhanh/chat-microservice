...
export const initializeSocket = (token: string): Socket | null => {
  ...
  // Connect to API Gateway with /chat namespace
  // Gateway automatically detects namespace and routes to Chat Service
  const gatewayUrl = import.meta.env.VITE_API_GATEWAY_URL || 'http://localhost:3000'
  socket = io(`${gatewayUrl}/chat`, {
    auth: {
      token,
    },
...
export const initializeNotificationSocket = (token: string) => {
  ...
  // Connect to API Gateway with /notifications namespace
  // Gateway automatically detects namespace and routes to Notification Service
  const gatewayUrl = import.meta.env.VITE_API_GATEWAY_URL || 'http://localhost:3000'
  notificationSocket = io(`${gatewayUrl}/notifications`, {
    auth: {
      token,
    },
    transports: ['websocket'],
