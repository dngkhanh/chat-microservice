...
export default function NotificationDropdown() {
  ...
  const handleMarkAsRead = async (notificationId: string) => {
    try {
      ...
      if (notifSocket && notifSocket.connected) {
        // Emit WebSocket event to mark as read
        notifSocket.emit(
          'notification:read',
          { notificationId },
          (response: any) => {
            if (response?.ok) {
              console.log('Notification marked as read via WebSocket:', notificationId)
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const notifSocket = getNotificationSocket()
      if (notifSocket && notifSocket.connected) {
        // Emit WebSocket event for each unread notification
        notifications
          .filter((n) => !(n.isRead ?? n.is_read ?? false))
  ...
  const handleDelete = async (notificationId: string) => {
    try {
      const notifSocket = getNotificationSocket()
      if (notifSocket && notifSocket.connected) {
        // Emit WebSocket event to delete notification
        notifSocket.emit(
          'notification:delete',
          { notificationId },
          (response: any) => {
            if (response?.ok) {
              console.log('Notification deleted via WebSocket:', notificationId)
